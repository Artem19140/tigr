...
const isPolling = computed(() => props.polling)
        ...
        >
            <v-card-text class="d-flex justify-space-between align-center py-4">
                ...
                <v-btn
                    variant="outlined"
                    rounded="lg"
                    @click="router.visit(comment.url)"
                    :disabled="comment.availability.disabled"
                >
                    Комментарий
                </v-btn>
            </v-card-text>
            ...
            >
                <template #item.actions="{ item }">
                    <EnrollmentConductActions
                        :enrollment="item"
                        :has-speaking="exam.data.hasSpeakingTasks"
                    />
                </template>
